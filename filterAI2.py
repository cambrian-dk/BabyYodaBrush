...
PATH = '/Users/darylkoh/Documents/school/hacknroll/2020/images/'
...
def load(image_file):
  ...
  w = w // 2
  real_image = tf.image.resize(image[:, w:, :], [256, 256],
                                method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
  input_image = tf.image.resize(image[:, :w, :], [256, 256],
                                method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def loadSingle(single_image_file_path):
  image = tf.io.read_file(single_image_file_path)
  image = tf.image.decode_jpeg(image)
  image = tf.image.resize(image, [256, 256],
                            method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
  image = tf.cast(image, tf.float32)
  print('SINGLE IMAGE SHAPE IS', image.get_shape())
  return image


inp, re = load(PATH+'train/3combined.jpg')
...
def generate_images(model, test_input, tar):
  print('INP SHAPE IS', test_input.get_shape())
  prediction = model(test_input, training=True)
  plt.figure(figsize=(15,15))

  display_list = [test_input[0], tar[0], prediction[0]]
  title = ['Input Image', 'Ground Truth', 'Predicted Image']
  print('prediction is type ', type(prediction[0]))
...
def process_image(model, img_path):
    single_dataset = tf.data.Dataset.list_files('/Users/darylkoh/Documents/school/hacknroll/2020/' + img_path)
    single_dataset = single_dataset.map([loadSingle(img_path)],
                                        num_parallel_calls=tf.data.experimental.AUTOTUNE)
    single_dataset = train_dataset.batch(BATCH_SIZE)
    for img in single_dataset.take(1):
...
if __name__ == '__main__':
    ...
    if args.mode == 'gen':
        ...
        # duplicate the local image
        jpeg = Image.open("images/test/local.jpg")
        newwidth = jpeg.size[0] * 2
        newheight = jpeg.size[1]
        new_img = Image.new('RGB', (newwidth, newheight))
        new_img.paste(jpeg, (0, 0))
        new_img.paste(jpeg, (jpeg.size[0], 0))
        new_img.save('images/test/localCombined.jpg')
        print("SAVED")
    ...
    if args.mode == 'process':
        checkpoint.restore(tf.train.latest_checkpoint(checkpoint_dir))
        process_image(generator, args.f)
